...
\author{Thomas K\"onig\footnote{koenig@epigenomics.com}, Klaus J\"unemann\footnote{kjuen@epigenomics.com}, and Matthias Burger\\Epigenomics AG}
\maketitle
\tableofcontents
\section*{Abstract}
...
\begin{Sinput}
testsuite.c2f <- defineTestSuite("c2f", dirs="/foo/bar",
...
Note that for executing just one test file there is also a shortcut in order to make test case execution as easy as possible:
 \begin{Sinput}
runTestFile("/foo/bar/runitc2f.r")
\end{Sinput}
...
\end{enumerate}
 ...
 We conclude this section with some further comments on various apects of the test execution framework:
\begin{itemize}
